...
instance.interceptors.request.use((config) => {
  if (getToken()) {
    ...
    // config.params = { icode: getToken() }; // 让每次请求携带token
  }
  ...
  } : { 'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8' }, config.headers);
...
instance.interceptors.response.use((response) => {
  // 对响应数据做处理
  // const { code } = response || {};
  // if (code === 1001) { // 4006: token无效，4007: token过期
  //   const instance = Message.error(response.msg);
  //   setTimeout(() => {
  //     instance.close();
